...
| 2026-08-15 | F8 | ✅ 完成 | 唯一实质缺陷在 McpServerManager：_configs（普通 List）被 UI 增删与后台枚举无锁并发访问，且有 AsReadOnly 包装泄漏与并发写盘风险。修复：新增 _configLock 覆盖全部 _configs 读写、GetServers 返回快照、SaveConfigs 全程持锁+事件锁外、构造函数支持注入配置路径（测试隔离）。CleanerStateStore（按文件粒度信号量+原子替换）与 AISharedContextService（锁内交换+克隆进出）审计达标，补线程模型注释。审计结论记录于 docs/singleton-thread-safety-audit.md |
| 2026-08-15 | 回归(F8) | ✅ 通过 | dotnet test 251/251 通过（248 + 新增 3 项并发测试：McpServerManager 并发增/枚举/删、CleanerStateStore 60 路并行自增无丢失更新、AISharedContextService 并行 Set/Get 快照一致性） |
| 2026-08-15 | F9 | ✅ 完成 | ①火忘调用：全仓扫出 21 处，18 处目标方法已内部兜底，修复 3 处（AICopilotPage.ScrollToBottomAsync 方法内兜底、MediaManagerViewModel.Library 元数据 Task.Run 外层兜底——CreateLinkedTokenSource 在 CTS 释放窗口可抛 ObjectDisposedException、AITaskCenterService 延迟保存 Task.Run 补 catch+日志）。②F9 原列注释 catch 与版本号两项经核实已随 F6/F5 达标（非 docs 无 1.0.5）。③F6 遗留单行 best-effort catch 收口：现存 23 处空 catch 全部处理，McpServerManager.LoadConfigs 升级为可选 ILogger 警告（配置损坏静默变空是真实可诊断性缺口），其余 22 处补注释说明可忽略原因；达标标准=日志或注释必居其一，复扫无裸空块 |
| 2026-08-15 | 回归(F9) | ✅ 通过 | dotnet test 251/251 通过，0 编译警告。fix-plan 九项（F1~F9）全部完成 |
